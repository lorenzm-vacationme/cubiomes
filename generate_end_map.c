...
int main(int argc, char *argv[]) {
    ...
    // Apply the specified seed to the generator for End
    applySeed(&g, DIM_END, seed);

    // Predefined range for generating biomes, centered around (0, 0) for the main island
    Range r;
    r.scale = 16;         // Scale for biome coordinates
    r.x = -300; r.z = -300; // Starting coordinates to center the map on (0, 0)
    r.sx = 600; r.sz = 600; // Size of the area to generate, covering a 600x600 block area
    ...
    char outputFile[256];
    snprintf(outputFile, sizeof(outputFile), "%s/overworld_seed_%lu.ppm", dirUrl, (unsigned long)seed);
    ...
    printf("End biome map generated and saved to %s\n", outputFile);
