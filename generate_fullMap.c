...
void *generateTilesForZoomLevel(void *arg) {
    ...
    int maxTileCount = params->tile_count / 2;

    setupGenerator(&g, MC_1_18, LARGE_BIOMES);

    // Generate the tiles in a spiral
    for (int i = 0; i < params->tile_count * params->tile_count; ++i) {
        // Only generate tiles within the bounds
        if (x >= -maxTileCount && x <= maxTileCount && y >= -maxTileCount && y <= maxTileCount) {
            generateTile(&g, params->seed, x, y, tileSize, params->outputDir, params->zoomLevel, params->scale);
        }

        // Update the coordinates for the spiral logic
        x += dx;
        y += dy;
        segmentPassed++;

        // If we've finished a segment, change direction
